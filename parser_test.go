...
func TestNormalizeFields(t *testing.T) {
	tests := []struct {
		name     string
		input    []string
		options  cron.ParseOption
		expected []string
	}{
		{
			"AllFields_NoOptional",
			[]string{"0", "5", "*", "*", "*", "*"},
			cron.Second | Minute | Hour | Dom | Month | Dow | Descriptor,
			[]string{"0", "5", "*", "*", "*", "*"},
		},
		{
...
func TestStandardSpecSchedule(t *testing.T) {
	...
	for _, c := range entries {
		actual, err := cron.ParseStandard(c.expr)
		if len(c.err) != 0 && (err == nil || !strings.Contains(err.Error(), c.err)) {
			t.Errorf("%s => expected %v, got %v", c.expr, c.err, err)
		}
		if len(c.err) == 0 && err != nil {
...
func every5min(loc *time.Location) *cron.SpecSchedule {
	return &cron.SpecSchedule{1 << 0, 1 << 5, all(hours), all(dom), all(months), all(dow), loc}
}

func every5min5s(loc *time.Location) *cron.SpecSchedule {
	return &cron.SpecSchedule{1 << 5, 1 << 5, all(hours), all(dom), all(months), all(dow), loc}
}

func midnight(loc *time.Location) *cron.SpecSchedule {
	return &cron.SpecSchedule{1, 1, 1, all(dom), all(months), all(dow), loc}
}

func annual(loc *time.Location) *cron.SpecSchedule {
