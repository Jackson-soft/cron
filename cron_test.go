...
func TestRunningMultipleSchedules(t *testing.T) {
	...
	crn := newWithSeconds()
	crn.AddFunc("0 0 0 1 1 ?", func() {})
	crn.AddFunc("0 0 0 31 12 ?", func() {})
	crn.AddFunc("* * * * * ?", func() { wg.Done() })
	crn.Schedule(cron.Every(time.Minute), cron.JobFunc(func() {}))
	crn.Schedule(cron.Every(time.Second), cron.JobFunc(func() { wg.Done() }))
	crn.Schedule(cron.Every(time.Hour), cron.JobFunc(func() {}))
...
func TestNonLocalTimezone(t *testing.T) {
	...
	cron := cron.New(cron.WithLocation(loc), cron.WithParser(secondParser))
	cron.AddFunc(spec, func() { wg.Done() })
	cron.Start()
	defer cron.Stop()
...
func TestJob(t *testing.T) {
	...
	crn := newWithSeconds()
	crn.AddJob("0 0 0 30 Feb ?", testJob{wg, "job0"})
	crn.AddJob("0 0 0 1 1 ?", testJob{wg, "job1"})
	job2, _ := crn.AddJob("* * * * * ?", testJob{wg, "job2"})
	crn.AddJob("1 0 0 1 1 ?", testJob{wg, "job3"})
	crn.Schedule(cron.Every(5*time.Second+5*time.Nanosecond), testJob{wg, "job4"})
	job5 := crn.Schedule(cron.Every(5*time.Minute), testJob{wg, "job5"})

	// Test getting an Entry pre-Start.
	if actualName := crn.Entry(job2).Job.(testJob).name; actualName != "job2" {
		t.Error("wrong job retrieved:", actualName)
	}
	if actualName := crn.Entry(job5).Job.(testJob).name; actualName != "job5" {
		t.Error("wrong job retrieved:", actualName)
	}
...
func TestScheduleAfterRemoval(t *testing.T) {
	...
	crn := newWithSeconds()
	hourJob := crn.Schedule(cron.Every(time.Hour), cron.JobFunc(func() {}))
	crn.Schedule(cron.Every(time.Second), cron.JobFunc(func() {
		mu.Lock()
		defer mu.Unlock()
		switch calls {
		case 0:
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	crn := newWithSeconds()
	var calls int64
	crn.AddFunc("* * * * * *", func() { atomic.AddInt64(&calls, 1) })
	crn.Schedule(new(ZeroSchedule), cron.JobFunc(func() { t.Error("expected zero task will not run") }))
	crn.Start()
	defer crn.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
