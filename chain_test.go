...
func appendingJob(slice *[]int, value int) cron.Job {
	var m sync.Mutex
	return cron.JobFunc(func() {
		m.Lock()
...
func appendingWrapper(slice *[]int, value int) cron.JobWrapper {
...
func TestChain(t *testing.T) {
	...
	cron.NewChain(append1, append2, append3).Then(append4).Run()
...
func TestChainRecover(t *testing.T) {
	panickingJob := cron.JobFunc(func() {
	...
	t.Run("panic exits job by default", func(t *testing.T) {
		...
		cron.NewChain().Then(panickingJob).
			Run()
	})
...
func TestChainDelayIfStillRunning(t *testing.T) {
	t.Run("runs immediately", func(t *testing.T) {
		var j countJob
		wrappedJob := cron.NewChain(cron.DelayIfStillRunning(cron.DiscardLogger)).Then(&j)
		go wrappedJob.Run()
...
func TestChainSkipIfStillRunning(t *testing.T) {
	t.Run("runs immediately", func(t *testing.T) {
		var j countJob
		wrappedJob := cron.NewChain(cron.SkipIfStillRunning(cron.DiscardLogger)).Then(&j)
		go wrappedJob.Run()
		time.Sleep(2 * time.Millisecond) // Give the job 2ms to complete.
		if c := j.Done(); c != 1 {
			t.Errorf("expected job run once, immediately, got %d", c)
